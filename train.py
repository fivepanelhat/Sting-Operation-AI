...
def check_data_config(data_cfg_path):
    ...
    try:
        ...
        resolved_path = data_path if os.path.isabs(data_path) else os.path.abspath(data_path)
        
        train_path = os.path.join(resolved_path, cfg.get('train', ''))
        val_path = os.path.join(resolved_path, cfg.get('val', ''))
        
        print(f"Resolving paths from dataset config:")
        ...
        if not os.path.exists(resolved_path):
            print(f"ERROR: Dataset base directory does not exist: {resolved_path}")
            return False
...
def main():
    parser = argparse.ArgumentParser(description="Sting Operation AI - Training Script")
    parser.add_argument("-d", "--data", type=str, default="config/data.yaml", help="Path to data.yaml config (default: config/data.yaml)")
    parser.add_argument("-e", "--epochs", type=int, default=50, help="Number of training epochs (default: 50)")
    parser.add_argument("-i", "--imgsz", type=int, default=640, help="Image size (default: 640)")
    parser.add_argument("-m", "--model", type=str, default="yolov8n.pt", help="Base model weights (default: yolov8n.pt)")
    parser.add_argument("-n", "--name", type=str, default="v4_final_run", help="Experiment name (default: v4_final_run)")
    parser.add_argument("-p", "--project", type=str, default="models/trained_models", help="Project save directory (default: models/trained_models)")
    parser.add_argument("--device", type=str, default="", help="Device to run on (e.g. cpu, cuda, or 0)")
