...
try:
    ...
    print("Error: Numpy is required to simulate raw frame processing allocations.")
    sys.exit(1)
...
def run_leak_test():
    print(f"Simulating heavy edge vision ingestion processing loop ({ITERATIONS} cycles)...")
    ...
    try:
        ...
        print(f"Memory processing test complete in {duration:.2f} seconds (Avg Frame Cycle: {(duration/ITERATIONS)*1000:.1f}ms).")
    except MemoryError:
        print("CRITICAL CRASH: Edge Vision memory threshold breached. Garbage collection failed.")
