...
try:
    ...
    print("[ERROR] Hailo Software Suite Compiler Toolchain not detected locally.")
    sys.exit(1)

def run_quantization_pipeline(onnx_model_path, calibration_data_dir, output_hef_name):
    """
    ...
    print("[SUCCESS] Model topology verified and parsed into local graph structures.")
    ...
    print("[OPTIMIZING] Injecting calibration dataset into compression layer...")
    ...
    print(f"[COMPILING] Forging final HEF engine payload to target: {output_path}")
    
    runner.compile(output_path)
    print(f"[MINT] Automated MLOps loop finished. Model registry updated.")
