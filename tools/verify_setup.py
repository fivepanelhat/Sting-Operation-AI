...
def verify_setup():
    ...
    if not os.path.exists(config_file):
        ...
        try:
            ...
            if cfg.get("path") != "data":
                errors.append(
                    f"data.yaml path is set to '{cfg.get('path')}', expected 'data'."
                )
            else:
            ...
            for k, v in expected_names.items():
                if names.get(k) != v:
                    errors.append(
                        f"data.yaml class mapping mismatch: expected {k}: {v}, got {k}: {names.get(k)}"
                    )
            if not errors:
    ...
    for split in ["train", "val"]:
        ...
        if stray_files:
            errors.append(
                f"Stray files found in images/{split}: {stray_files}"
            )
        else:
    ...
    for split in ["train", "val"]:
        ...
        stray_files = [
            f for f in os.listdir(lbl_split_dir) if not f.endswith(".txt")
        ]
        if stray_files:
            errors.append(
                f"Stray files found in labels/{split}: {stray_files}"
            )
            ...
            if file.endswith(".txt"):
                ...
                for line_idx, line in enumerate(lines):
                    ...
                    if parts:
                        ...
                        if cls not in ["0", "1", "2"]:
                            errors.append(
                                f"Invalid class index {cls} found in label file: data/labels/{split}/{file} on line {line_idx+1}"
                            )
    ...
    if os.path.exists(lbl_top_dir):
        top_files = [
            f
            for f in os.listdir(lbl_top_dir)
            if os.path.isfile(os.path.join(lbl_top_dir, f))
        ]
        if top_files:
            errors.append(
                f"Stray files found in top-level labels/ directory: {top_files}"
            )
        else:
    ...
    if os.path.exists(venv_dir) or os.path.exists(shared_venv_dir):
        ...
        warnings.append(
            ".venv directory is missing (checked local and shared stack-wide)! Run local setup first."
        )
    ...
    if not os.path.exists(model_path):
        warnings.append(
            f"Trained model v3 weights not found at: {os.path.relpath(model_path, base_dir)}"
        )
    else:
