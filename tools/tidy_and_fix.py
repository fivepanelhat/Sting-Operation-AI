    ...
    if os.path.exists(images_train_dir):
        for file in os.listdir(images_train_dir):
            if file.endswith('.txt'):
                ...
                is_tracked = run_git_cmd(['ls-files', f"data/images/train/{file}"])
                
                if is_tracked:
                    # Move via git mv to keep history/cleanliness
                    run_git_cmd(['mv', f"data/images/train/{file}", f"data/raw_annotations/{file}"])
                    print(f"Git moved: data/images/train/{file} -> data/raw_annotations/{file}")
                else:
                    shutil.move(src, dst)
                    print(f"Moved: data/images/train/{file} -> data/raw_annotations/{file}")
                moved_txt_count += 1
    ...
    if os.path.exists(labels_dir):
        for file in os.listdir(labels_dir):
            if file.lower().endswith('.png'):
                ...
                if is_tracked:
                    run_git_cmd(['mv', f"data/labels/{file}", f"data/visualizations/{file}"])
                    print(f"Git moved: data/labels/{file} -> data/visualizations/{file}")
                else:
                    shutil.move(src, dst)
                    print(f"Moved: data/labels/{file} -> data/visualizations/{file}")
                moved_png_count += 1
