...
def install_roboflow():
    ...
    try:
        ...
        try:
            ...
            print(
                "Failed to install roboflow using uv, trying standard pip..."
            )
            python_exe = sys.executable
            subprocess.run(
                [python_exe, "-m", "pip", "install", "roboflow"], check=True
            )
...
def clean_local_dataset(data_dir):
    ...
    for folder in folders_to_remove:
        if os.path.exists(folder):
            ...
            print(
                f"  Removed: {os.path.relpath(folder, os.path.dirname(data_dir))}"
            )
    ...
    for split in splits:
        ...
        if os.path.exists(src_split_img):
            ...
            for file in os.listdir(src_split_img):
                shutil.copy(
                    os.path.join(src_split_img, file),
                    os.path.join(dest_split_img, file),
                )
            print(
                f"  Copied {len(os.listdir(src_split_img))} images to images/{dest_split}"
            )
        ...
        if os.path.exists(src_split_lbl):
            ...
            for file in os.listdir(src_split_lbl):
                shutil.copy(
                    os.path.join(src_split_lbl, file),
                    os.path.join(dest_split_lbl, file),
                )
            print(
                f"  Copied {len(os.listdir(src_split_lbl))} labels to labels/{dest_split}"
            )
...
def main():
    ...
    if not api_key:
        ...
        if not api_key:
            print(
                "ERROR: Private API Key is required to download the dataset."
            )
            sys.exit(1)
    ...
    try:
        ...
        dataset = project.version(1).download(
            "yolov8", location=os.path.join(base_dir, "temp_download")
        )
    ...
    subprocess.run(
        [sys.executable, os.path.join(base_dir, "tools", "verify_setup.py")]
    )
